...
impl CPU {
    ...
    fn drw_xyn(&mut self, x: usize, y: usize, len: u8) {
        ...
        //get a slice from the main memory which symbolizes the sprite to be drawn
        let sprite = &self.memory[i as usize..(i + len as u16) as usize];
        for b in sprite {
            let mut display_index = 0;
            let a = b.to_bits();
            display_index += 8;
            &self.display[x * y + display_index - 8..x * y + display_index].copy_from_slice(&a);
        }
    }
...
fn main() {
    ...
    let mut file = File::open("pong1.ch8").unwrap();
